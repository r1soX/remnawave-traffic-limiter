...
`resetWhiteTraffic=true` передаётся только при начале нового расчётного периода. Имя поля сохранено для обратной совместимости, но limiter обрабатывает его как единый сброс пары: параллельно отправляет Remnawave сброс для Main и технического WhiteList-пользователя, а затем синхронизирует их состояние. Для смены тарифа и докупки трафика передаётся `false`: новый лимит применяется, но накопленный расход WhiteList не сбрасывается. Если расход уже больше нового лимита, WhiteList корректно переводится на сквад уведомления.
