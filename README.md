...
При начале нового расчётного периода limiter одним управляющим запросом запускает параллельный сброс нативных счётчиков Main и технического WhiteList-пользователя. Смена тарифа и докупка трафика расход не обнуляют.
Патч также учитывает модель синхронизации Bedolaga v4.10+ «панель — источник
истины»: технический безлимит Main не затирает фактический лимит пары в базе и
не отображается как безлимит в кабинете.
...
Текущий патч проверен с Bedolaga **v4.15.0** (`877690a`). Подробная установка,
